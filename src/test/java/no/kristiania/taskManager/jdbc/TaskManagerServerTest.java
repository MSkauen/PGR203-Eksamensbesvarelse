...
public class TaskManagerServerTest {
    ...
    void shouldHandlePostRequests() throws IOException {
        HttpClientRequest client = new HttpClientRequest("localhost", server.getPort(), "/api/members?/addMember", "name=Guro&age=24");
        assertEquals(302, client.executePost().getStatusCode());
    }

    @Test
    void shouldHandleListRequest() throws IOException {
        HttpClientRequest client = new HttpClientRequest("localhost", server.getPort(), "/api/members?/listMembers=Option");
        assertEquals(200, client.executeGet().getStatusCode());
    }

    @Test
    void shouldReturnInternalServerError() throws IOException {
        HttpClientRequest client = new HttpClientRequest("localhost", server.getPort(), "/api/members?/addMember", "foo=bar&bar=foo");
